...
# other constants
PATH_NAME = "//home//pi//python//weather_word//"  # set path to find apiboot.txt and log.txt files
...
def parseWeatherData(strip, obj):
    ...
    for i in range(OBJMAX):
        temp[i] = str(obj["location"]["current"]["forecast"]["forecastday"]["day"]["astro"][i]["hour"]["temp_f"])
        humid[i] = str(obj["location"]["current"]["forecast"]["forecastday"]["day"]["astro"][i]["hour"]["humidity"])
        wind[i] = str(obj["location"]["current"]["forecast"]["forecastday"]["day"]["astro"][i]["hour"]["wind_mph"])
        fct[i] = str(obj["location"]["current"]["forecast"]["forecastday"]["day"]["astro"][i]["hour"]["condition"]["code"])
        fcttime[i] = str(obj["location"]["current"]["forecast"]["forecastday"]["day"]["astro"][i]["hour"]["time_epoch"])
    ...
    while success == False:
        ...
        if success == True:            
            try:
                ...
                writeLogFile('\n\n' + str(apiUrl), 'a')
                writeLogFile('\n\nWeather data provided by Weather API, LLC (WUL)', 'a')
        ...
        if success == True:
            try:
                ...
                error = str(obj["forecast"])
                success = True
            except:
                # utilize yellow color wipe to signal error and increment failed loop count
                success = False
                failedLoopCount += 1
                writeLogFile('\n\nAPI failed to provide forecast data after attempt after pulling data ' + str(failedLoopCount) + '.', 'a')
                writeLogFile('\nProgram will terminate after ' + str(MAX_FAIL_LOOP_COUNT) + ' consecutive attempts.', 'a')
                writeLogFile('\nTrying again in ' + str(TIME_BETWEEN_FAILED) + ' seconds.', 'a')
                colorWipeRand(strip, [170,170,0])                
